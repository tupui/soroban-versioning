...
- **Outcome Contracts** – optional contracts to invoke on each outcome (approved, rejected, cancelled), each with a target address, function name, and arguments
...
After the voting window closes any maintainer can press **Execute**. If the proposal includes outcome contracts, the contract matching the result index is invoked (Approved = index 0, Rejected = index 1, Cancelled = index 2).

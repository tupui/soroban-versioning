...
/// Get the name bytes from a string.
///
/// # Arguments
/// * `env` - The environment object
/// * `name` - The name to get the bytes from
///
/// # Returns
/// * `Bytes` - The name bytes
///
/// # Panics
/// * If the name is longer than 15 bytes
pub fn get_name_bytes(env: &Env, name: &String) -> Bytes {
...
/// Get the project key from a name.
///
/// # Arguments
/// * `env` - The environment object
/// * `name` - The name to get the project key from
///
/// # Returns
/// * `Bytes` - The project key
///
/// # Panics
/// * If the name is longer than 15 bytes
pub fn get_project_key(env: &Env, name: &String) -> Bytes {
    let name_b = get_name_bytes(env, name);
    env.crypto().keccak256(&name_b).into()
}

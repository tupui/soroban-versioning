...
impl DaoTrait for Tansu {
    ...
    fn create_proposal(
        ...
        let _ = token_stellar
            ...
            .unwrap_or_else(|_| {
                panic_with_error!(&env, &errors::ContractErrors::InsufficientBalance)
            });
    ...
    fn vote(env: Env, voter: Address, project_key: Bytes, proposal_id: u32, vote: types::Vote) {
        ...
        let _ = token::TokenClient::new(&env, &token_address)
            .try_transfer(&voter, env.current_contract_address(), &amount)
            .unwrap_or_else(|_| {
                panic_with_error!(&env, &errors::ContractErrors::InsufficientBalance);
            });
        // Record the vote
    ...
    fn execute(
        ...
        let _ = token::TokenClient::new(&env, &sac_contract.address) // xlm
            .try_transfer(
                &env.current_contract_address(),
                &proposal.proposer,
                &PROPOSAL_COLLATERAL,
            )
            .unwrap_or_else(|_| panic_with_error!(&env, &errors::ContractErrors::CollateralError));
        ...
        for vote_ in &proposal.vote_data.votes {
            ...
            let _ = token::TokenClient::new(&env, &transfer_contract)
                .try_transfer(&env.current_contract_address(), vote_address, &amount)
                .unwrap_or_else(|_| {
                    panic_with_error!(&env, &errors::ContractErrors::CollateralError)
                });
        }

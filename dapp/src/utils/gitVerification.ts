...
export async function fetchGitPublicKeys(gitHandle: string): Promise<string[]> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch keys from ${provider.name}: ${response.statusText}`,
      );
    }
...
export function parseSEP53Envelope(envelope: string): {
  ...
  if (lines.length !== 5) {
    throw new Error(
      "Invalid SEP-53 envelope format: must have exactly 5 lines",
    );
  }
  ...
  if (!header || header !== "Stellar Signed Message") {
    throw new Error(
      'Invalid SEP-53 envelope: header must be "Stellar Signed Message"',
    );
  }
  ...
  if (!payload || !payload.startsWith("tansu-bind|")) {
    throw new Error(
      'Invalid SEP-53 envelope: payload must start with "tansu-bind|"',
    );
  }
    ...
    if (!hashes.sha512Async) {
      (hashes as any).sha512Async = async (...msgs: Uint8Array[]) => {
        const combined = new Uint8Array(
          msgs.reduce((len, m) => len + m.length, 0),
        );
        let offset = 0;
        for (const m of msgs) {
...
export function generateSSHSignCommand(
  envelope: string,
  privateKeyPath?: string,
): string {
  const keyPath = privateKeyPath || "~/.ssh/id_ed25519";
  return `echo -n "${envelope.replace(/"/g, '\\"')}" | ssh-keygen -Y sign -f ${keyPath} -n file -O hashalg=sha256 /dev/stdin`;
}
  ...
  try {
    ...
    function readUint32(): number {
      if (offset + 4 > data.length)
        throw new Error("Buffer underrun reading uint32");
    ...
    function readBuffer(): Buffer {
      ...
      if (offset + len > data.length)
        throw new Error("Buffer underrun reading buffer");
    ...
    if (rawSignature.length !== 64) {
      throw new Error(
        `Expected 64-byte Ed25519 signature, got ${rawSignature.length} bytes`,
      );
    }
...
async function createSSHSIGVerifyBlob(
  ...
  if (hashAlgorithm === "sha512") {
    ...
    throw new Error(
      `Unsupported hash algorithm for wrapping: ${hashAlgorithm}`,
    );
  }
  ...
  const writeString = (s: string | Buffer | Uint8Array): Uint8Array => {
    ...
    return new Uint8Array(
      Buffer.concat([new Uint8Array(len), new Uint8Array(content)]),
    );
  };
